...
export class MigrationManager {
  ...
  async dropDatabase(
    ...
    if (!force) {
      throw new Error('Database drop operation requires force=true to prevent accidental deletion');
    }
  ...
  async migrateMaster(): Promise<MigrationResult> {
    ...
    const masterMigrationsPath = path.join(process.cwd(), 'src/core/database/migrations/master');
  ...
  async migrateTenant(
    ...
    let databaseName = dbName || `tenant_${tenantId}`;
    if (config?.connectionString) {
      // Try to extract db name from connection string for logging
      try {
        const url = new URL(config.connectionString);
        databaseName = url.pathname.replace(/^\//, '') || databaseName;
      } catch {}
    }
    logger.info(`=== Starting Tenant Database Migration: ${databaseName} ===`);

    const tenantMigrationsPath = path.join(process.cwd(), 'src/core/database/migrations/tenant');
  ...
  async migrateAllTenants(
    ...
    for (const tenant of tenants) {
      try {
        ...
        const result = await this.migrateTenant(tenant.tenantId, undefined, config);
        results.push(result);

        if (result.success) {
          logger.info(`✓ Tenant '${tenant.tenantId}' migrated successfully`);
        } else {
          logger.error(`✗ Tenant '${tenant.tenantId}' migration failed: ${result.error}`);
        }
      } catch (err) {
        logger.error(`Error migrating tenant '${tenant.tenantId}':`, err);
        results.push({
          success: false,
